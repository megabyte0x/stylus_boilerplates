...
use stylus_sdk::{abi::Bytes, call::MutatingCallContext, prelude::*, stylus_core::calls};
...
impl SendEther {
    ...
    pub fn send_via_transfer(&self, to: Address) -> Result<(), Vec<u8>> {
        self.vm().transfer_eth(to, self.vm().msg_value())?;
        Ok(())
    }
    ...
    pub fn send_via_call(&mut self, to: Address) -> Result<(), Vec<u8>> {
        let host = self.vm().msg_sender();

        let context: &dyn MutatingCallContext;
        self.vm().call(self, to, &[]);
        Ok(())
    }
    ...
    pub fn send_via_call_with_call_data(
        ...
        call(
            Call::new_in(self).value(self.vm().msg_value()),
            to,
            data.as_slice(),
        )?;
        Ok(())
